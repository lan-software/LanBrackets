...
const statusColor: Record<string, string> = {
    ...
    registration_closed: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300',
    ...
    <div>
        ...
        <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
            <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <div class="mb-8 flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ competitions.total }} competition{{ competitions.total !== 1 ? 's' : '' }}
                        </p>
                    </div>
                ...
                <div v-if="competitions.data.length === 0" class="rounded-lg border border-dashed border-gray-300 p-12 text-center dark:border-gray-700">
                    <p class="text-gray-500 dark:text-gray-400">No competitions yet.</p>
                </div>
                ...
                <div v-else class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    >
                        <div class="mb-3 flex items-start justify-between gap-3">
                            <h2 class="text-lg font-semibold text-gray-900 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400">
                                {{ competition.name }}
                            </h2>
                            <span
                                class="shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium"
                                :class="statusColor[competition.status] ?? 'bg-gray-100 text-gray-700'"
                            >
                        ...
                        <div class="mb-4 flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <span class="inline-flex items-center gap-1">
                                ...
                                {{ competition.stages_count }} stage{{ competition.stages_count !== 1 ? 's' : '' }}
                            </span>
                        </div>

                        <div class="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500">
                            <span class="rounded bg-gray-100 px-2 py-0.5 font-medium dark:bg-gray-700">
                                {{ typeLabel[competition.type] ?? competition.type }}
                            </span>
                ...
                <!-- Pagination -->
                <div v-if="competitions.last_page > 1" class="mt-8 flex items-center justify-center gap-2">
                    <Link
                    ...
                    <span class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                        Page {{ competitions.current_page }} of {{ competitions.last_page }}
                    </span>
                    <Link
